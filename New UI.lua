pcall(function()
    -- ตรวจสอบและลบ UI เก่า หากมีอยู่ เพื่อป้องกัน UI ซ้อนกัน
...
function Library:Window(title)
    ...
    -- ปุ่ม "ซ่อน UI" (Hide UI Button) - เพิ่มเข้ามาใหม่
    local HideUI = Instance.new("ImageButton")
    HideUI.Name = "HideUI"
    HideUI.Parent = Top
    HideUI.AnchorPoint = Vector2.new(1, 0.5)
    HideUI.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
    HideUI.BackgroundTransparency = 1.000
    -- ปรับตำแหน่งให้อยู่ข้างๆ ปุ่ม Close
    HideUI.Position = UDim2.new(1, -30, 0.5, 0) -- -6 (Close) - 20 (size) - 4 (spacing) = -30
    HideUI.Size = UDim2.new(0, 20, 0, 20)
    -- ใช้รูปไอคอนที่สื่อถึงการซ่อน/แสดง (อาจจะเป็นลูกศรชี้ลง/ขึ้น หรือขีดสามขีด)
    -- ตัวอย่างนี้ใช้ไอคอนลูกศรชี้ลงเป็นค่าเริ่มต้น
    HideUI.Image = "rbxassetid://3926307971" -- ไอคอน Roblox, จะใช้ ImageRectOffset เพื่อเลือกรูป
    HideUI.ImageRectOffset = Vector2.new(204, 364) -- ไอคอนลูกศรชี้ลง (เหมือนปุ่ม Resize)
    HideUI.ImageRectSize = Vector2.new(36, 36)
    HideUI.ImageColor3 = Color3.fromRGB(199, 199, 199)
    HideUI.ScaleType = Enum.ScaleType.Crop

    HideUI.MouseButton1Click:Connect(function()
        ui.Enabled = not ui.Enabled -- สลับสถานะ Enabled ของ ScreenGui หลัก
        if ui.Enabled then
            print("UI Shown")
            -- อาจเปลี่ยนไอคอนเป็นลูกศรชี้ขึ้นเมื่อ UI แสดงอยู่
            TweenService:Create(HideUI, TweenInfo.new(0.2), {ImageRectOffset = Vector2.new(204, 324)}):Play() -- ไอคอนลูกศรชี้ขึ้น
        else
            print("UI Hidden")
            -- เปลี่ยนไอคอนเป็นลูกศรชี้ลงเมื่อ UI ถูกซ่อน
            TweenService:Create(HideUI, TweenInfo.new(0.2), {ImageRectOffset = Vector2.new(204, 364)}):Play() -- ไอคอนลูกศรชี้ลง
        end
    end)

    HideUI.MouseEnter:Connect(function()
        TweenService:Create(HideUI, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {ImageColor3 = Color3.fromRGB(255,255,255)}):Play()
    end)
    
    HideUI.MouseLeave:Connect(function()
        TweenService:Create(HideUI, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {ImageColor3 = Color3.fromRGB(166, 166, 166)}):Play()
    end)
    -- Mobile support
    HideUI.TouchTap:Connect(function()
        HideUI.MouseButton1Click:Fire()
    end)
